...
// which are part of the Whatcoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="whatcoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /whatcoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "whatcoin-whatcoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from whatcoin-whatcoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'whatcoin-whatcoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from whatcoin-whatcoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /whatcoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /whatcoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /whatcoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /whatcoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /whatcoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /whatcoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /whatcoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
