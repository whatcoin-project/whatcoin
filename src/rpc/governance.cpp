...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 WHATCOIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on whatcoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in whatcoin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("whatcoin.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in whatcoin.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Whatcoin features */
    { "whatcoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "whatcoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "whatcoin",               "gobject",                &gobject,                true,  {} },
    { "whatcoin",               "voteraw",                &voteraw,                true,  {} },
