...
# whatcoin_hash
RUN git clone https://github.com/whatcoin-project/whatcoin_hash
RUN cd whatcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} whatcoin
RUN useradd -u ${USER_ID} -g whatcoin -s /bin/bash -m -d /whatcoin whatcoin
...
RUN mkdir /whatcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /whatcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /whatcoin-src
